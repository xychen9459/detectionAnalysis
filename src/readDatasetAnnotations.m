...
switch lower(dataset)
  case 'voc'
    ...
    % no cached annotation data, read directly from VOC, 
    % and if possible, add extra info
    if ~exist(outfn, 'file')
      ...
      for r = 1:numel(rec)  % for each image
        for n = 1:numel(rec(r).objects)   % for each object
      ...
      % truth
      usediff = true;
      for o = 1:numel(dataset_params.objnames_all)  % for each object class
